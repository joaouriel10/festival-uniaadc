...
const ratingSchema = z.object({
  ...
  regionalMusic: z.object({
    choralCategory: z.object({
      vocalTuning: z.number().min(0.5).max(10),
      vocalHarmony: z.number().min(0.5).max(10),
      technicalLevel: z.number().min(0.5).max(10),
      performanceCreatividade: z.number().min(0.5).max(10),
    }),
    instrumental: z.object({
      musicTechnicalLevel: z.number().min(0.5).max(10),
      arrangementCoherence: z.number().min(0.5).max(10),
      overallPerformance: z.number().min(0.5).max(10),
    }),
